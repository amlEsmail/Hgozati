...
export class HotelSearchComponent implements OnInit ,OnDestroy{
  ...
  constructor(private router: Router, calendar: NgbCalendar, private myapi: MyApiService, private carruncy: CurruncyServiceService, private datePipe: DatePipe,private cdRef:ChangeDetectorRef) {
    ...
    console.log('next 7-day', this.toDate)
    ...
    console.log("checkin:"+this.formday+"checkout:"+ this.today);
  }
...
// detect change after rooms btn changes
  ...
  ngOnInit() {
    ...
    //  this.hotelcities = this.myapi.getHotelsCities();
    //  this.extractcites(this.hotelcities);
...
// console.log("novalidatiy",input.value);
// console.log("nationArr",this.nationArr);
  ...
  onSubmit() {
    ...
    if (this.hotelSearch.valid) {
      ...
      // this.router.navigate(['/hotelResult']);
